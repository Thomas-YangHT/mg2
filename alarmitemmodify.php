...
function  alarmitemrela(){
         if(isset($_POST['addalarmitem'])){     
             addalarmitem();
         }
         if(isset($_POST['modalarmitem'])){  
             mysqlgetalarmitem();   
             updatealarmitem();
         }
         if(isset($_POST['delalarmitem'])){     
             mysqlgetalarmitem();
             delalarmitem();
         }
         mysqlgetalarmitem();
         show_part0();
         show_alarmitem_part1();
} 

function mysqlgetalarmitem(){
  global $sqlSVR;
  global $sqlPASS;
  global $sqlUSER;
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($item, SORT_ASC, $alarmitems);
...
function addalarmitem(){ 
  ...
  $SQL = "insert into  alarmitem(item,value,`explain`) values('','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." addalarmitem sql:".$SQL);
...
function updatealarmitem(){ 
  ...
  for ($i=0; $i<count($alarmitems); $i++) {
      if( !empty($_REQUEST["id".$alarmitems[$i]['Id']]) ){
           $SQL = "update alarmitem set ";
           $SQL.= " item=\"".$_POST[id.$alarmitems[$i]['Id'].'item']."\",";
           $SQL.= " value=\"".$_POST[id.$alarmitems[$i]['Id'].'value']."\",";
           $SQL.= " `explain`=\"".$_POST[id.$alarmitems[$i]['Id'].'explain']."\""; 
           $SQL.= " where Id=\"".$alarmitems[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updatealarmitem sql:".$SQL);
...
function delalarmitem(){ 
  ...
  for ($i=0; $i<count($alarmitems); $i++) {
      if( !empty($_REQUEST["id".$alarmitems[$i]['Id']]) ){
           $SQL = "delete from alarmitem where Id=\"".$alarmitems[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." delalarmitem sql:".$SQL);
...
function  show_alarmitem_part1(){
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:30%'><div id='rkey'>item:</div></li>\n";
     echo "	<li style='width:30%'><div id='rkey'>value:</div></li> \n";		
     echo "	<li style='width:38%'><div id='rkey'>explain:</div></li> \n";

	 echo " </br>\n";

     for ($i=0; $i<count($alarmitems); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$alarmitems[$i]['Id']."' />\n";
            echo "	<li style='width:30%'><div id='rvalue'><input style='width:200%' type='text' name='id".$alarmitems[$i]['Id']."item' value='".$alarmitems[$i]['item']."'/></div></li>\n";
         	echo "	<li style='width:30%'><div id='rvalue'><input style='width:200%' type='text' name='id".$alarmitems[$i]['Id']."value' value='".$alarmitems[$i]['value']."'/></div></li> \n";		
         	echo "	<li style='width:38%'><div id='rvalue'><input style='width:300%' type='text' name='id".$alarmitems[$i]['Id']."explain' value='".$alarmitems[$i]['explain']."' /></div></li> \n";
		
		    echo " </br>\n";
     } 
     echo " </br>\n";
     echo "<input name='addalarmitem' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='modalarmitem' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='delalarmitem' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
