...
function  dutyassignrela(){
         if(isset($_POST['adddutyassign'])){     
             adddutyassign();
         }
         if(isset($_POST['moddutyassign'])){  
             mysqlgetdutyassign();   
             updatedutyassign();
         }
         if(isset($_POST['deldutyassign'])){     
             mysqlgetdutyassign();
             deldutyassign();
         }
         mysqlgetdutyassign();
         show_part0();
         show_dutyassign_part1();
} 

function mysqlgetdutyassign(){
  global $sqlSVR;
  global $sqlPASS;
  global $sqlUSER;
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($name, SORT_ASC, $dutyassigns);

   mysql_free_result($res);
   mysql_close($con);
} 

function adddutyassign(){ 
  global $sqlSVR;
  global $sqlPASS;
  global $sqlUSER;
  ...
  $SQL = "insert into  dutyassign(name,email,alarmitem) values('','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." adddutyassign sql:".$SQL);
...
function updatedutyassign(){ 
  ...
  for ($i=0; $i<count($dutyassigns); $i++) {
      if( !empty($_REQUEST["id".$dutyassigns[$i]['Id']]) ){
           $SQL = "update dutyassign set ";
           $SQL.= " name=\"".$_POST[id.$dutyassigns[$i]['Id'].'name']."\",";
           $SQL.= " email=\"".$_POST[id.$dutyassigns[$i]['Id'].'email']."\",";
           $SQL.= " alarmitem=\"".$_POST[id.$dutyassigns[$i]['Id'].'alarmitem']."\""; 
           $SQL.= " where Id=\"".$dutyassigns[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updatedutyassign sql:".$SQL);
...
function deldutyassign(){ 
  ...
  for ($i=0; $i<count($dutyassigns); $i++) {
      if( !empty($_REQUEST["id".$dutyassigns[$i]['Id']]) ){
           $SQL = "delete from dutyassign where Id=\"".$dutyassigns[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." deldutyassign sql:".$SQL);
...
function  show_dutyassign_part1(){
     global $dutyassigns;
     ...
     echo "<span class='navPoint' id='switchPoint1' title='关闭/打开'>5</span>报警人员编辑：</font>\n";
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:30%'><div id='rkey'>name:</div></li>\n";
     echo "	<li style='width:30%'><div id='rkey'>email:</div></li> \n";		
     echo "	<li style='width:38%'><div id='rkey'>alarmitem:</div></li> \n";

	 echo " </br>\n";

     for ($i=0; $i<count($dutyassigns); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$dutyassigns[$i]['Id']."' />\n";
            echo "	<li style='width:30%'><div id='rvalue'><input style='width:200%' type='text' name='id".$dutyassigns[$i]['Id']."name' value='".$dutyassigns[$i]['name']."'/></div></li>\n";
         	echo "	<li style='width:30%'><div id='rvalue'><input style='width:200%' type='text' name='id".$dutyassigns[$i]['Id']."email' value='".$dutyassigns[$i]['email']."'/></div></li> \n";		
         	echo "	<li style='width:38%'><div id='rvalue'><input style='width:300%' type='text' name='id".$dutyassigns[$i]['Id']."alarmitem' value='".$dutyassigns[$i]['alarmitem']."' /></div></li> \n";
		
		    echo " </br>\n";
     } 
     echo " </br>\n";
     echo "<input name='adddutyassign' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='moddutyassign' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='deldutyassign' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
