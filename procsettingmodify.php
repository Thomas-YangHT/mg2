...
function  procrela(){
         ...
         if(isset($_POST['modproc'])){  
             mysqlgetproc();   
             updateproc();
         }
         if(isset($_POST['delproc'])){     
             mysqlgetproc();
             delproc();
         }
         mysqlgetproc();
...
function mysqlgetproc(){
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($project, SORT_ASC, $procsettings);
...
function addproc(){ 
  ...
  $SQL = "insert into  procsetting(ip,procname,startscript,stopscript,port,status,projname) values('','','','','','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." addproc sql:".$SQL);
...
function updateproc(){ 
  ...
  for ($i=0; $i<count($procsettings); $i++) {
      if( !empty($_REQUEST["id".$procsettings[$i]['Id']]) ){
           $SQL = "update procsetting set ";
           $SQL.= " ip=\"".$_POST[id.$procsettings[$i]['Id'].'ip']."\",";
           $SQL.= " procname=\"".$_POST[id.$procsettings[$i]['Id'].'procname']."\",";
           $SQL.= " startscript=\"".$_POST[id.$procsettings[$i]['Id'].'startscript']."\","; 
           $SQL.= " stopscript=\"".$_POST[id.$procsettings[$i]['Id'].'stopscript']."\","; 
           $SQL.= " port=\"".$_POST[id.$procsettings[$i]['Id'].'port']."\",";
           $SQL.= " status=\"".$_POST[id.$procsettings[$i]['Id'].'status']."\","; 
           $SQL.= " projname=\"".$_POST[id.$procsettings[$i]['Id'].'projname']."\""; 
           $SQL.= " where Id=\"".$procsettings[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updateproc sql:".$SQL);
...
function delproc(){ 
  ...
  for ($i=0; $i<count($procsettings); $i++) {
      if( !empty($_REQUEST["id".$procsettings[$i]['Id']]) ){
           $SQL = "delete from procsetting where Id=\"".$procsettings[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." delproc sql:".$SQL);
...
function  show_proc_part1(){
     ...
     echo "<span class='navPoint' id='switchPoint1' title='关闭/打开'>5</span>主机项目编辑：</font>\n";
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:10%'><div id='rkey'>Ip:</div></li>\n";
     echo "	<li style='width:10%'><div id='rkey'>Procname:</div></li> \n";		
     echo "	<li style='width:20%'><div id='rkey'>Startscript:</div></li> \n";
     echo "	<li style='width:20%'><div id='rkey'>Stopscript:</div></li> \n";	 
     echo "	<li style='width:7%'><div id='rkey'>Port:</div></li> \n";
     echo "	<li style='width:13%'><div id='rkey'>Status:</div></li> \n";
     echo "	<li style='width:18%'><div id='rkey'>Projname:</div></li> \n";		
	 echo " </br>\n";

     for ($i=0; $i<count($procsettings); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$procsettings[$i]['Id']."' />\n";
            echo "	<li style='width:10%'><div id='rvalue'><input style='width:110%' type='text' name='id".$procsettings[$i]['Id']."ip' value='".$procsettings[$i]['ip']."'/></div></li>\n";
         	echo "	<li style='width:10%'><div id='rvalue'><input style='width:120%' type='text' name='id".$procsettings[$i]['Id']."procname' value='".$procsettings[$i]['procname']."'/></div></li> \n";		
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:200%' type='text' name='id".$procsettings[$i]['Id']."startscript' value='".$procsettings[$i]['startscript']."' /></div></li> \n";
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:200%' type='text' name='id".$procsettings[$i]['Id']."stopscript' value='".$procsettings[$i]['stopscript']."'/></div></li> \n";	
         	echo "	<li style='width:7%'><div id='rvalue'><input style='width:100%' type='text' name='id".$procsettings[$i]['Id']."port' value='".$procsettings[$i]['port']."' /></div></li> \n";
         	echo "	<li style='width:13%'><div id='rvalue'><input style='width:150%' type='text' name='id".$procsettings[$i]['Id']."status' value='".$procsettings[$i]['status']."'/></div></li> \n";
         	echo "	<li style='width:18%'><div id='rvalue'><input style='width:150%' type='text' name='id".$procsettings[$i]['Id']."projname' value='".$procsettings[$i]['projname']."'/></div></li> \n";		
		
		    echo " </br>\n";
     } 
     echo " </br>\n";
     echo "<input name='addproc' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='modproc' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='delproc' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
