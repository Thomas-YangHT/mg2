...
function  pstatusetrela(){
         if(isset($_POST['addpstatuset'])){     
             addpstatuset();
         }
         if(isset($_POST['modpstatuset'])){  
             mysqlgetpstatuset();   
             updatepstatuset();
         }
         if(isset($_POST['delpstatuset'])){     
             mysqlgetpstatuset();
             delpstatuset();
         }
         mysqlgetpstatuset();
         show_part0();
         show_pstatuset_part1();
} 

function mysqlgetpstatuset(){
  global $sqlSVR;
  global $sqlPASS;
  global $sqlUSER;
  ...
  $SQL = "SELECT * from portstatussetting order by id";
  $res=mysql_query($SQL);
  global $pstatusets;
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($id, SORT_ASC, $pstatusets);

   mysql_free_result($res);
   mysql_close($con);
} 

function addpstatuset(){ 
  global $sqlSVR;
  global $sqlPASS;
  global $sqlUSER;
  ...
  $SQL = "insert into  portstatussetting(ip,monif,id) values('','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." addpstatuset sql:".$SQL);
...
function updatepstatuset(){ 
  ...
  for ($i=0; $i<count($pstatusets); $i++) {
      if( !empty($_REQUEST["id".$pstatusets[$i]['Idd']]) ){
           $SQL = "update portstatussetting set ";
           $SQL.= " ip=\"".$_POST[id.$pstatusets[$i]['Idd'].'ip']."\",";
           $SQL.= " monif=\"".$_POST[id.$pstatusets[$i]['Idd'].'monif']."\",";
           $SQL.= " id=\"".$_POST[id.$pstatusets[$i]['Idd'].'id']."\""; 
           $SQL.= " where Idd=\"".$pstatusets[$i]['Idd']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updatepstatuset sql:".$SQL);
...
function delpstatuset(){ 
  ...
  for ($i=0; $i<count($pstatusets); $i++) {
      if( !empty($_REQUEST["id".$pstatusets[$i]['Idd']]) ){
           $SQL = "delete from portstatussetting where Idd=\"".$pstatusets[$i]['Idd']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." delpstatuset sql:".$SQL);
...
function  show_pstatuset_part1(){
     ...
     echo "<span class='navPoint' id='switchPoint1' title='关闭/打开'>5</span>IP端口监控信息编辑：</font>\n";
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:35%'><div id='rkey'>ip:</div></li>\n";
     echo "	<li style='width:43%'><div id='rkey'>monif:</div></li> \n";		
     echo "	<li style='width:20%'><div id='rkey'>id:</div></li> \n";
	 echo " </br>\n";

     for ($i=0; $i<count($pstatusets); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$pstatusets[$i]['Idd']."' />\n";
            echo "	<li style='width:35%'><div id='rvalue'><input style='width:350%' type='text' name='id".$pstatusets[$i]['Idd']."ip' value='".$pstatusets[$i]['ip']."'/></div></li>\n";
         	echo "	<li style='width:43%'><div id='rvalue'><input style='width:400%' type='text' name='id".$pstatusets[$i]['Idd']."monif' value='".$pstatusets[$i]['monif']."'/></div></li> \n";		
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:300%' type='text' name='id".$pstatusets[$i]['Idd']."id' value='".$pstatusets[$i]['id']."' /></div></li> \n";
		    echo " </br>\n";
     } 
     echo " <li style='width:100%'>&nbsp&nbsp</li>\n";
	 echo "<li style='width:100%'> &nbsp&nbsp说明：id表示分组号，与端口信息编辑页的id相关联； monif与端口信息编辑页的ports一一对应，1表示监控该端口，0表示不监控该端口；</li>";
	 echo " <li style='width:100%'>&nbsp&nbsp</li>\n";
     echo "<input name='addpstatuset' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='modpstatuset' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='delpstatuset' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
