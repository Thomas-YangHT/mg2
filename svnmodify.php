...
function  svnrela(){
         if(isset($_POST['addsvn'])){     
             addsvn();
         }
         if(isset($_POST['modsvn'])){  
             mysqlgetsvn();   
             updatesvn();
         }
         if(isset($_POST['delsvn'])){     
             mysqlgetsvn();
             delsvn();
         }
         mysqlgetsvn();
...
function mysqlgetsvn(){
  ...
  while($row=mysql_fetch_array($res))  {
   ...
   array_multisort($project, SORT_ASC, $svnprojects);
...
function addsvn(){ 
  ...
  $SQL = "insert into  svnproject(projname,repodir,mvndir,dstlocation,url,dkprojname,type) values('','','','','','','')";
  writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." addsvn sql:".$SQL);
...
function updatesvn(){ 
  ...
  for ($i=0; $i<count($svnprojects); $i++) {
      if( !empty($_REQUEST["id".$svnprojects[$i]['Id']]) ){
           $SQL = "update svnproject set ";
           $SQL.= " projname=\"".$_POST[id.$svnprojects[$i]['Id'].'projname']."\",";
           $SQL.= " repodir=\"".$_POST[id.$svnprojects[$i]['Id'].'repodir']."\",";
           $SQL.= " mvndir=\"".$_POST[id.$svnprojects[$i]['Id'].'mvndir']."\","; 
           $SQL.= " dstlocation=\"".$_POST[id.$svnprojects[$i]['Id'].'dstlocation']."\","; 
           $SQL.= " dkprojname=\"".$_POST[id.$svnprojects[$i]['Id'].'dkprojname']."\",";
           $SQL.= " url=\"".$_POST[id.$svnprojects[$i]['Id'].'url']."\","; 
           $SQL.= " type=\"".$_POST[id.$svnprojects[$i]['Id'].'type']."\""; 
           $SQL.= " where Id=\"".$svnprojects[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." updatesvn sql:".$SQL);
...
function delsvn(){ 
  ...
  for ($i=0; $i<count($svnprojects); $i++) {
      if( !empty($_REQUEST["id".$svnprojects[$i]['Id']]) ){
           $SQL = "delete from svnproject where Id=\"".$svnprojects[$i]['Id']."\"";
           writelog(date("Y.m.d H:i:sa l")." user: ".$_SESSION['user']." from ".$_SERVER['REMOTE_ADDR']." delsvn sql:".$SQL);
...
function  show_svn_part1(){
     ...
     echo "<span class='navPoint' id='switchPoint1' title='关闭/打开'>5</span>SVN项目编辑：</font>\n";
	 ...
	 echo "	<li style='width:2%'>&nbsp&nbsp</li><li style='width:10%'><div id='rkey'>Projname:</div></li>\n";
     echo "	<li style='width:15%'><div id='rkey'>Repodir:</div></li> \n";		
     echo "	<li style='width:15%'><div id='rkey'>Mvndir:</div></li> \n";
     echo "	<li style='width:20%'><div id='rkey'>Dstlocation:</div></li> \n";
     echo "	<li style='width:20%'><div id='rkey'>Url:</div></li> \n";		
     echo "	<li style='width:10%'><div id='rkey'>Dkprojname:</div></li> \n";
     echo "	<li style='width:8%'><div id='rkey'>Type:</div></li> \n";	 
	 echo " </br>\n";

     for ($i=0; $i<count($svnprojects); $i++) {
            echo " <input type='checkbox' style='float:left' name='id".$svnprojects[$i]['Id']."' />\n";
            echo "	<li style='width:10%'><div id='rvalue'><input style='width:110%' type='text' name='id".$svnprojects[$i]['Id']."projname' value='".$svnprojects[$i]['projname']."'/></div></li>\n";
         	echo "	<li style='width:15%'><div id='rvalue'><input style='width:200%' type='text' name='id".$svnprojects[$i]['Id']."repodir' value='".$svnprojects[$i]['repodir']."'/></div></li> \n";		
         	echo "	<li style='width:15%'><div id='rvalue'><input style='width:200%' type='text' name='id".$svnprojects[$i]['Id']."mvndir' value='".$svnprojects[$i]['mvndir']."' /></div></li> \n";
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:250%' type='text' name='id".$svnprojects[$i]['Id']."dstlocation' value='".$svnprojects[$i]['dstlocation']."'/></div></li> \n";
         	echo "	<li style='width:20%'><div id='rvalue'><input style='width:250%' type='text' name='id".$svnprojects[$i]['Id']."url' value='".$svnprojects[$i]['url']."'/></div></li> \n";		
         	echo "	<li style='width:10%'><div id='rvalue'><input style='width:110%' type='text' name='id".$svnprojects[$i]['Id']."dkprojname' value='".$svnprojects[$i]['dkprojname']."' /></div></li> \n";
         	echo "	<li style='width:8%'><div id='rvalue'><input style='width:50%' type='text' name='id".$svnprojects[$i]['Id']."type' value='".$svnprojects[$i]['type']."'/></div></li> \n";			
		    echo " </br>\n";
     } 
     echo " </br>\n";
     echo "<input name='addsvn' class='shiny-blue' type='submit' value='增加' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='modsvn' class='shiny-blue' type='submit' value='修改' onclick=''/> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n";
     echo "<input name='delsvn' class='shiny-blue' type='submit' value='删除' onclick=''/> \n";     
     echo "</form>\n"; 
     echo "</ul> \n";
     echo "</div> \n";
